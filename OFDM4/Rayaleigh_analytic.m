...
figure(Ch+1), clf %Clears all children of the specified figure 
...
Lch=Delay(end)+1;           % Channel length, end takes the last value of array
Nbps=2;                     % QAM selector 
...
fid=fopen(file_name, 'w+');  %Opens a file with specified name 
...
for i=0:length(EbN0)
   ...
   for m=1:N_iter
      ...
      Neb=Neb+sum(sum(int2bit(X_r,Nbps)~=int2bit(X,Nbps)));
